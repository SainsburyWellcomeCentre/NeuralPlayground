...
import re
from sehec.extras.TEM_helper_functions import *
...
def get_data(save_dirs, run, date, recent=-1):
    ...
    a_rnn = np.load(save_path + '/A_RNN_' + index + '.npy', allow_pickle=True)
    # g2g = np.load(save_path + '/g2g_' + index + '.npy', allow_pickle=True)
    x_all = np.load(save_path + '/x_all_' + index + '.npy', allow_pickle=True)
    g_all = np.load(save_path + '/g_all_' + index + '.npy', allow_pickle=True)
    p_all = np.load(save_path + '/p_all_' + index + '.npy', allow_pickle=True)
    p_gen_all = np.load(save_path + '/p_gen_all_' + index + '.npy', allow_pickle=True)
    acc_s_t_to = np.load(save_path + '/acc_s_t_to_' + index + '.npy', allow_pickle=True)
    acc_s_t_from = np.load(save_path + '/acc_s_t_from_' + index + '.npy', allow_pickle=True)
    positions = np.load(save_path + '/pos_count_' + index + '.npy', allow_pickle=True)
    # shinys = np.load(save_path + '/shiny_states_' + index + '.npy', allow_pickle=True)
    # adj = np.load(save_path + '/adj_' + index + '.npy', allow_pickle=True)

    g_all = np.nan_to_num(g_all)
    p_all = np.nan_to_num(p_all)

    pars = np.load(save_path + '/params.npy', allow_pickle=True)
    params = pars.item()

    print(int(index) * params['t_episode'])
    widths = np.load(save_path + '/widths_' + index + '.npy', allow_pickle=True)
    ...
    g_timeseries = np.load(save_path + '/gs_timeseries_' + index + '.npy', allow_pickle=True)
    try:
        p_timeseries = np.load(save_path + '/ps_timeseries_' + index + '.npy', allow_pickle=True)
    except FileNotFoundError:
        p_timeseries = None
        print('no p_timeseries')
    pos_timeseries = np.load(save_path + '/pos_timeseries_' + index + '.npy', allow_pickle=True)
    timeseries = (g_timeseries, p_timeseries, pos_timeseries)

    data = (a_rnn, x_all, g_all, p_all, p_gen_all, acc_s_t_to, acc_s_t_from, positions, timeseries)
    para = (params, widths, batch_id, g_size, p_size, s_size, s_size_comp, n_freq, width, states)

    return data, para, list_of_files, save_path


def sort_data(g_all, p_all, widths, mult, smoothing, params, batch_id, g_max_0=False, p_max_0=True):
