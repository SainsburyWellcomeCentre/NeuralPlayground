...
from utils import rng_sequence_from_rng, set_device
...
def run(config_path, config):
    ...
    plot_input_target_output(
        ...
        4,
        edege_lables,
        os.path.join(save_path, "in_out_targ.pdf"),
    )
    plot_message_passing_layers(
        list(graph_test.nodes.sum(-1)),
        outputs[1],
        ...
        edege_lables,
        os.path.join(save_path, "message_passing_graph.pdf"),
    )
    ...
    plot_graph_grid_activations(
        outputs[0].nodes.tolist(),
        graph_test,
        os.path.join(save_path, "outputs.pdf"),
    ...
    plot_graph_grid_activations(
        list(graph_test.nodes.sum(-1)),
        graph_test,
        os.path.join(save_path, "Inputs.pdf"),
