    ...
    def _forward(x):
        ...
        for n in range(num_message_passing_steps):
            if add_residual:
                ...
                x = message_passing_layer(
                    x, edge_mlp_sizes, node_mlp_sizes, use_layer_norm
                )
                x = x._replace(
                    nodes=x.nodes + previous_x.nodes, edges=x.edges + previous_x.edges
                )
