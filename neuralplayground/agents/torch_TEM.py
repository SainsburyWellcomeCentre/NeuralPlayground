...
from neuralplayground.agents.TEM_extras.torch_TEM_parameters import *
...
class Model(torch.nn.Module):
    ...
    def act(self, iter, step, locations, observations, actions):
        # Initialise TEM
        if iter == 0:
            self.initialise()

        # Get updated parameters for this backprop iteration
        self.eta_new, self.lambda_new, self.p2g_scale_offset, self.lr, self.walk_length_center, self.loss_weights = parameter_iteration(iter, self.hyper)
        # Update eta and lambda
        self.hyper['eta'] = self.eta_new
        self.hyper['lambda'] = self.lambda_new
        # Update scaling of offset for variance of inferred grounded position
        self.hyper['p2g_scale_offset'] = self.p2g_scale_offset
        ...
        if iter % self.hyper['save_interval'] == 0 and iter > 0:
            gs_all, ps_all = [None] * self.hyper['n_envs_save'], [None] * self.hyper['n_envs_save']

        steps = self.forward(locations=locations, observations=observations, actions=actions, test=False)
        ...
        for ind, step in enumerate(steps):
            ...
            for env_i, env_visited in enumerate(self.visited):
                if env_visited[step.g[env_i]['id']]:
                    step_loss.append(self.loss_weights * torch.stack([l[env_i] for l in step.L]))
                else:
            ...
            # Save internal variables relevant for plotting
            gs.append(torch.concat(step.g_inf, axis=1))
            ps.append(torch.concat(step.p_inf, axis=1))

        for j, a in enumerate(gs):
        ...
        self.prev_iter = [steps[-1].detach()]

        if iter % self.hyper['save_interval'] == 0 and iter > 0:
            save_data = [gs, ps, locations]
            prev_cell_maps = [gs_all, ps_all]

            cell_list, locations = prepare_data_maps(save_data, prev_cell_maps, locations, self.hyper)
            gs_all, ps_all = cell_list

            torch.save(self.state_dict(), self.model_path + '/tem_' + str(iter) + '.pt')
            torch.save(self.hyper, self.model_path + '/params_' + str(iter) + '.pt')
            # save TEM variables (for plotting)
            np.save(self.save_path + '/' + 'g_all_' + str(iter), gs_all)
            np.save(self.save_path + '/' + 'p_all_' + str(iter), ps_all)
            final_i = iter
            save_needed, save_ticker = False, False

        if iter == self.hyper['train_it'] and step == self.hyper['n_walks'][iter]:
            # Save the final state of the model after training has finished
            torch.save(self.state_dict(), self.model_path + '/tem_' + str(iter) + '.pt')
            torch.save(self.hyper, self.model_path + '/params_' + str(iter) + '.pt')

            np.save(self.save_path + '/' + 'positions_' + str(final_i + 1), self.walks)
            np.save(self.save_path + '/' + 'pos_count_' + str(final_i + 1), self.pos_count)
            np.save(self.save_path + '/' + 'widths_' + str(final_i + 1), self.hyper['widths'])

        # Compute model accuracies
        acc_p, acc_g, acc_gt = np.mean([[np.mean(a) for a in step.correct()] for step in steps], axis=0)
        acc_p, acc_g, acc_gt = [a * 100 for a in (acc_p, acc_g, acc_gt)]

    def initialise(self):
        # Create directories for storing all information about the current run
        self.run_path, self.train_path, self.model_path, self.save_path, self.script_path, self.envs_path = make_directories()
        # Save all python files in current directory to script directory
        shutil.copy2('/nfs/nhome/live/lhollingsworth/Documents/NeuralPlayground/NPG/EHC_model_comparison/neuralplayground/agents/torch_TEM.py', os.path.join(self.model_path, 'torch_TEM'))
        # Save parameters
        np.save(os.path.join(self.save_path, 'params'), self.hyper)
        ...
        self.logger = make_logger(self.run_path)
        # Make an ADAM optimizer for TEM
        self.adam = torch.optim.Adam(self.parameters(), lr=self.hyper['lr_max'])
        # Initialise whether a state has been visited for each world
        self.visited = [[False for _ in range(self.hyper['n_states_world'][env])] for env in range(self.hyper['batch_size'])]
        ...
        self.states = [self.hyper['n_states_world'][self.hyper['diff_env_batches_envs'][env]] for env in range(self.hyper['batch_size'])]
        self.pos_count = []

    def forward(self, walks=None, locations=None, observations=None, actions=None, prev_M=None, test=False):
        if not test:
            for env in range(self.hyper['batch_size']):
                pos = [locations[step][env].get('id') for step in range(self.hyper['n_rollout'])]
                self.pos_count.append(np.bincount(pos, minlength=self.states[env]) + 0.001)
            # Collect all information in walk variable
            walk = [[locations[i], torch.from_numpy(np.reshape(observations, (20,16,45))[i]).type(torch.float32), np.reshape(actions, (20,16))[i].tolist()] for i in range(self.hyper['n_rollout'])]
        if test:
            walk = walks
            self.prev_iter = None
        # The previous iteration may contain walks without action. These are new walks, for which some parameters need to be reset.
        steps = self.init_walks(self.prev_iter)
    ...
    def loss(self, g_gen, p_gen, x_logits, x, g_inf, p_inf, p_inf_x, M_prev):
        ...
        L_p_x = torch.sum(torch.stack(squared_error(p_inf, p_inf_x), dim=0), dim=0) if self.hyper[
        ...
        L_x_gen = cross_entropy(x_logits[2], labels)
        # L_x_g: Losses generated by generative model from g_inf -> p -> x
        L_x_g = cross_entropy(x_logits[1], labels)
        # L_x_p: Losses generated by generative model from p_inf -> x
        L_x_p = cross_entropy(x_logits[0], labels)
    ...
    def inf_g(self, p_x, g_gen, x, locations):
        ...
        for f in range(self.hyper['n_f']):
            if self.hyper['use_p_inf']:
                ...
                mu, sigma = inv_var_weight([mu_g_path[f], mu_g_mem[f]], [sigma_g_path[f], sigma_g_mem[f]])
            else:
        ...
        if any(shiny_envs):
            ...
            for f in range(module_start, self.hyper['n_f']):
                ...
                mu, sigma = inv_var_weight([mu_g[f][shiny_envs, :], mu_g_shiny[f - module_start]],
    ...
    def f_n(self, x):
        ...
        normalised = [normalise(relu(x[f] - torch.mean(x[f]))) for f in range(self.hyper['n_f'])]
        return normalised
    ...
    def f_p(self, p):
        ...
        activation = [leaky_relu(torch.clamp(p_f, min=-1, max=1)) for p_f in p] if type(
            p) is list else leaky_relu(torch.clamp(p, min=-1, max=1))
        return activation
