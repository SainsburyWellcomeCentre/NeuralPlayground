...
Learning place cells, grid cells and invariances with excitatory and inhibitory
plasticity
...
class Weber2018(RatMovementAgent):
    """Implementation for Weber and Sprekeler 2018
    Learning place cells, grid cells and invariances with excitatory and inhibitory
    plasticity https://doi.org/10.7554/eLife.34560.001
    ...
    inh_cell_list: ndarray
        Ni x (self.resolution**2) with tuning curves for each inhibitory neuron and
        each pixel in the 2D array
    exc_cell_list: ndarray
        Ne x (self.resolution**2) with tuning curves for each excitatory neuron and
        each pixel in the 2D array
    wi: ndarray
    ...
    inh_rates_functions: list
        List of parameters of Gaussian functions in tuning curves for each inhibitory
        neuron
    exc_rates_functions: list
        List of parameters of Gaussian functions in tuning curves for each excitatory
        neuron
    ...
    generate_tuning_curves(self, n_curves, cov_scale, Nf, alpha)
        ...
        for a given number of neurons, covariance, number of Gaussian per tuning curve
        and height
    ...
    get_rates(self, cell_list, pos)
        Get firing rate if input neurons at the given position by finding the pixel
        that is closest
    ...
    update(self)
        Update weights using hebbian plasticity according to equation 2 for excitatory
        weights and equation 3 for inhibitory weights for a given position
    full_average_update(self, exc_normalization=True)
        Update weights using hebbian plasticity according to equation 2 for excitatory
        weights and equation 3 for inhibitory weights for all available positions
        in the grid
    full_update(self, exc_normalization=True)
        Update weights using hebbian plasticity according to equation 2 for excitatory
        weights and equation 3, but using self.update function as a sub routine
    ...
    def __init__(
        ...
        **mod_kwargs,
        ...
        """Refer to Table 1 and Table 2 from the paper for best parameter
        selection.
        ...
        Parameters of the model from Weber and Sprekeler 2018
            ...
            Nef: int
                Number of Gaussians in the spatial tuning of excitatory neurons
                (Table 1)
            Nif: int
                Number of Gaussians in the spatial tuning of inhibitory neurons
                (Table 2)
            alpha_e: float
                Gaussian height of Gaussian functions in spatial tuning for
                excitatory neurons (equation 12)
            alpha_i: float
                Gaussian height of Gaussian functions in spatial tuning for
                excitatory neurons (equation 12)
            we_init: float
            ...
            sigma_exc: float
                Standard deviation of Gaussian functions in excitatory tuning curves
                (equation 12)
            sigma_inh: float
                Standard deviation of Gaussian function in inhibitory tuning curves
                (equation 12)
            ro: float
                target rate for inhibitory neurons (equation 3)
            room_width: float
                room width specified by the environment
                (see examples/testing_weber_model.ipynb)
            room_depth: float
        ...
        self.resolution_depth = int(
            self.resolution * (self.room_depth / self.room_width)
        )
    ...
    def reset(self):
        """Initialize weights, tuning curves and global counter for weights
        updates done.
        """
        ...
        self.init_we_sum = np.sqrt(
            np.sum(self.we**2) + 1e-8
        )  # Keep track of normalization constant

    def generate_tuning_curves(
        self, n_curves: int, cov_scale: float, Nf: int, alpha: float
    ):
        """Generate tuning curves as a combination of Gaussians for a given
        number of neurons, covariance, number of Gaussian per tuning curve and
        height.
        ...
        cov_scale: float
            Standard deviation of Gaussian functions of each neuron tuning curves
            (exc_sigma or inh_sigma in __init__)
        Nf: int
            Number of Gaussians in the spatial tuning of each neuron
            (Nfe or Nfi in __init__)
        alpha: float
            Height of Gaussian functions in spatial tuning for neurons
            (alpha_i or alpha_e in __init__)
        ...
        cell_list: numpy ndarray
            n_curves x (self.resolution**2) with tuning curves for each neuron and
            each pixel in the 2D array

        """
        width_limit = self.room_width / 2.0
        depth_limit = self.room_depth / 2.0
        # This will become a numpy array with the tuning curves in 2D (images)
        cell_list = []
        ...
        for i in tqdm(range(n_curves), disable=self.disable_tqdm):
            ...
            for j in range(Nf):
                ...
                mean1 = np.random.uniform(
                    -width_limit * (1 + 0.2), width_limit * (1 + 0.2)
                )  # Sample means
                mean2 = np.random.uniform(
                    -depth_limit * (1 + 0.2), depth_limit * (1 + 0.2)
                )
                room_scale = np.max(np.array([self.room_width, self.room_width]))
                cov = np.diag(
                    np.multiply(cov_scale, np.array([room_scale, room_scale])) ** 2
                    + 1e-8
                )
                ...
                )  # normalization constant (eq 13)
                cell_i += (
                    rv.pdf(self.xy_combinations) * normalization_constant * alpha
                )  # See equation 13
    ...
    def get_output_rates(self, pos: np.ndarray):
        """Compute firing rate of the output neuron for the specific position,
        equation 1.
    ...
    def get_rates(self, cell_list: np.ndarray, pos: np.ndarray):
        """Get firing rate if input neurons at the given position by finding
        the pixel that is closest to the position given.

        Parameters
        ----------
        cell_list : numpy ndarray
            n_curves x (self.resolution**2) with tuning curves for each neuron and each
        ...
        -------
        rout : ndarray
            (number_of_neurons, ) array with the output firing rate for each of the
            tuning curves at position pos
        ...
        input_rates = np.clip(
            input_rates, a_min=0, a_max=np.amax(input_rates)
        )  # negatives to zero
        return input_rates

    def get_full_output_rate(self):
        """It computes equation 1 (or 4) for all available pixels at the same
        time taking advantage of broadcasting.
    ...
    def update(self, exc_normalization: bool = True, pos: np.ndarray = None):
        """Update weights using hebbian plasticity according to equation 2 for
        excitatory weights and equation 3 for inhibitory weights for a given
        position.
        ...
        if len(pos) == 0:
            ...
            self.we = np.clip(
                self.we, a_min=0, a_max=np.amax(self.we)
    ...
    def full_average_update(self, exc_normalization: bool = True):
        """Update weights using hebbian plasticity according to equation 2 for
        excitatory weights and equation 3 for inhibitory weights for all
        ...
        # Inhibitory weights update (eq 3)
        delta_wi = (
            self.etainh * (self.inh_cell_list @ (r_out - self.ro)) / self.resolution**2
        )
    ...
    def full_update(self, exc_normalization: bool = True):
        """Update weights using hebbian plasticity according to equation 2 for
        excitatory weights and equation 3, but using self.update function as a
        sub routine.
        ...
        xy_array = self.xy_combinations[
            random_permutation, :
        ]  # All points (x, y) in the grid
    ...
    def plot_all_rates(self, save_path: str = None, ax: mpl.axes.Axes = None):
        ...
        r_out_im = self.get_full_output_rate()
        r_out_im = r_out_im.reshape((self.resolution_width, self.resolution_depth))
        exc_im = self.exc_cell_list[
            np.random.choice(np.arange(self.exc_cell_list.shape[0])), ...
        ].reshape((self.resolution_width, self.resolution_depth))
        inh_im = self.inh_cell_list[
            np.random.choice(np.arange(self.inh_cell_list.shape[0])), ...
        ].reshape((self.resolution_width, self.resolution_depth))
        make_plot_rate_map(
            exc_im.T, ax[0], "Exc rates", "width", "depth", "Firing rate"
        )
        make_plot_rate_map(
            inh_im.T, ax[1], "Inh rates", "width", "depth", "Firing rate"
        )
        make_plot_rate_map(
            r_out_im.T, ax[2], "Out rate", "width", "depth", "Firing rate"
        )
