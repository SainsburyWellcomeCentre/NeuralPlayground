...
def plot_weights(models, params=None, steps=None, do_save=False):
    ...
    for param in params:
        ...
        for i in range(len(steps) - 1):
            ...
            axs[1, i].set_title(str(steps[i]) + ' to ' + str(steps[i + 1]) + ', ' + '{:.2E}'.format(
                np.mean(np.abs(values[i + 1] - values[i])) / (steps[i + 1] - steps[i])))
        # On the very last axis: plot the total difference between the first and the last
        axs[1, -1].imshow(values[-1] - values[0])
        axs[1, -1].set_title(
            str(steps[0]) + ' to ' + str(steps[-1]) + ', ' + '{:.2E}'.format(np.mean(np.abs(values[-1] - values[0]))))
...
def transform_coordinates(coords):
    ...
    for walk in coords:
        transformed_walk = []
        for step in walk:
            x_old, y_old = step[0], step[1]
            x_new = (x_old + 5) / 10
            y_new = (y_old + 5) / 10
            transformed_step = [x_new, y_new]
            transformed_walk.append(transformed_step)
        transformed_coords.append(transformed_walk)
    return transformed_coords

def plot_actions(environment, field='probability', ax=None, min_val=None, max_val=None, num_cols=100,
                 action_cm='viridis'):
    # If min_val and max_val are not specified: take the minimum and maximum of the supplied values
    min_val = min([action[field] for location in environment.locations for action in
                   location['actions']]) if min_val is None else min_val
    max_val = max([action[field] for location in environment.locations for action in
                   location['actions']]) if max_val is None else max_val
    # Create color map for locations: colour given by value input
    ...
    for i, location in enumerate(environment.locations):
        ...
        location_patches.append(plt.Circle((x_new, y_new), radius, color=[0, 0, 0]))
        ...
        for a, action in enumerate(location['actions']):
            ...
            if action['probability'] > 0:
                ...
                # Find where this action takes you
                locations_to = [environment.locations[loc_to] for loc_to in
                                np.where(np.array(action['transition']) > 0)[0]]
                # Create an action patch for each possible transition for this action
                for loc_to in locations_to:
...
def plot_walk(environment, walk, max_steps=None, n_steps=1, ax=None):
    ...
    location_patches = [patch_i for patch_i, patch in enumerate(ax.patches) if
                        type(patch) is plt.Circle or type(patch) is plt.Rectangle]
    ...
    prev_loc = np.array([environment[0][walk[0][0]['id']]['x'], environment[0][walk[0][0]['id']]['y']])
    # Run through walk, creating lines
    for step_i in range(1, max_steps, n_steps):
        x_new = (environment[0][walk[step_i][0]['id']]['x'] + 5) / 10
        y_new = (environment[0][walk[step_i][0]['id']]['y'] + 5) / 10
        # Get location of current location, with some jitter so lines don't overlap
        new_loc = np.array([x_new, y_new])
        # new_loc = np.array([environment[0][walk[step_i][0]['id']]['x'], environment[0][walk[step_i][0]['id']]['y']])
