...
from utils import convert_jraph_to_networkx_graph
...
def get_activations_graph_n(node_colour, graph, number_graph_batch=0,):
...
def get_node_pad(graph,i):
    ...
    return node_padd
def plot_graph_grid_activations(node_colour, graph,save_path,title,edege_lables, number_graph_batch=0,):
...
def plot_message_passing_layers_units(activations,targets,outputs,graph,number_hidden,n_message_passing,edege_lables,save_path):
    # minim 2 otherwise it breaks
...
def plot_roc(auc_roc, path,title ):
    fig = plt.figure(figsize=(5, 3))
    ax = fig.add_subplot(111)
    ax.title.set_text(title)
    ax.plot(auc_roc)
