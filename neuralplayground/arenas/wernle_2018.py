...
class Wernle2018(Hafting2008):
    ...
    set_room(self, room_id: str):
        Place the agent in the right room configuration depending the amount of
        exploration time
    ...
    ----------
    merge_time: float
        Time in minutes to remove the middle wall in the experiment, merging the
        two rooms
    switch_time: float
    ...
    def __init__(
        ...
        **env_kwargs,
        ...
        recording_index: int
            if None, load data from default recording index of corresponding
            experiment class
    ...
    def set_room(self, room_id: str):
        """Place the agent in the right room configuration depending the amount
        of exploration time.

        Parameters
        ----------
        room_id: str
            Either "A", "B" or "AB". If "A" or "B" it will locate the agent in one of
            the rooms with a wall in between.
            If "AB", it will place the agent in a random position and remove the wall
            in between
    ...
    def step(
        ...
        """Set the right room configuration, then call default step
        function.
        """
...
class MergingRoom(Simple2D):
    """Arena resembling Wernle2018 experimental setting BUT WITH GENERIC ROOM
    DIMENSIONS.
    ...
    def __init__(
        ...
        **env_kwargs,
        ...
        experiment_class:
            Experiment class to be initialized
        merge_time: float
            Time in minutes to remove the middle wall in the experiment,
    ...
    def set_room(self, room_id: str):
        ...
        if room_id == self.A_id:
            ...
            # Add wall in between to separete rooms
            self.custom_walls = [
                np.array([[self.arena_limits[0, 0], 0], [self.arena_limits[0, 1], 0]])
            ]
            # Update walls in the environment
            self.wall_list = self.default_walls + self.custom_walls
        elif room_id == self.B_id:
