...
class Hafting2008(Simple2D):
    ...
    step(self, action):
       Increment the global step count of the agent in the environment and updates the
       position of the agent according
    ...
    def __init__(
        ...
        **env_kwargs,
        ...
        recording_index: int
            if None, load data from default recording index of corresponding experiment
            class
        ...
        env_kwargs: dict
            Leave empty in this class, the arena parameters and sampling values are set
            to resemble the experiment. For full control over these parameters use
            Simple2D class
        ...
        env_kwargs["time_step_size"] = (
            1 / 50
        )  # Taken from experiment, 50 Hz movement sampling
    ...
    def reset(self, random_state: bool = False, custom_state: np.ndarray = None):
        """Reset the environment variables. If using behavioral data, it will
        reset the position to the initial position of the trajectory recorded
        in the experiment.
        ...
        -------
        observation: ndarray
            Because this is a fully observable environment, make_observation returns
            the state of the environment. Array of the observation of the agent in the
            environment (Could be modified as the environments are evolves)
    ...
    def set_animal_data(
        ...
        """Set position and head direction to be used by the Arena Class, See
        neuralplayground.experiments classes.
        """
        self.experiment.set_animal_data(
            recording_index=recording_index, tolerance=tolerance
        )
        if keep_history:
    ...
    def show_data(self, full_dataframe: bool = False):
        ...
        recording_list: Pandas dataframe
            List of available experiment, columns with rat_id, recording session and
            recorded variables
    ...
    def plot_recording_tetr(
        ...
        """Check plot_recording_tetrode method from
        neuralplayground.experiments.Hafting2008Data.
        """
        return self.experiment.plot_recording_tetr(
            recording_index, save_path, ax, tetrode_id, bin_size
        )
    ...
    def recording_tetr(
        ...
        neuralplayground.experiments.Hafting2008Data.
        """
        return self.experiment.tetrode_ratemap(
    ...
    def plot_recorded_trajectory(
        ...
        """Check plot_trajectory method from
        neuralplayground.experiments.Hafting2008Data.
        """
    ...
    def step(
        ...
        some action, return observation, new state and reward. If using
        behavioral data, the action argument is ignored, and instead inferred
        from the recorded trajectory in the experiment.
        ...
        normalize_step: bool
            If true, the action is normalized to have unit size, then scaled by
            the agent step size
        skip_every: int
            When using behavioral data, the next state will be the position and
            head direction
            "skip_every" recorded steps after the current one, essentially
        ...
        # In this case, the action is ignored and computed from the step in behavioral
