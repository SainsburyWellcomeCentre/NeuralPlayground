...
class Environment(Env):
    ...
    global_time: float
        time simulating environment through step method, then global_time =
        time_step_size * global_steps
    ...
    __init__(self, environment_name: str = "Environment", **env_kwargs):
        Initialize the class. env_kwargs arguments are specific for each of the child
         environments and
        described in their respective class
    make_observation(self):
        Returns the current state of the environment
    step(self, action):
        Runs the environment dynamics. Given some action, return observation,
    ...
    def make_observation(self):
        """Just take the state and returns an array of sensory information In
        more complex cases, the observation might be different from the
        ...
        self.state:
            Define within each subclass for specific environments
            Variable containing the state of the environment
            (eg. position in the environment)
    ...
    def step(self, action=None):
        ...
        observation:
            Define within each subclass for specific environments
            Any set of observation that is made by the agent in the environment
            (position, visual features,...)
        self.state:
            Define within each subclass for specific environments
            Variable containing the state of the environment
            (eg.position in the environment)
    ...
    def save_environment(self, save_path: str, raw_object: bool = True):
        """Save current variables of the object to re-instantiate the
        environment later.
        ...
        raw_object: bool
            ...
            env.restore_environment(save_path)
    ...
    def restore_environment(self, restore_path: str):
        ...
        ----------
        restore_path: str
            Path to retrieve the environment saved using save_environment method
            (raw_object=False)
    ...
    def __eq__(self, other):
        """Check if two environments are equal by comparing all of its
        attributes.
    ...
    def reward_function(self, action, state):
        """Reward curriculum as a function of action, state and attributes of
        the environment.
...
if __name__ == "__main__":
    env = Environment(
        environment_name="test_environment", time_step_size=0.5, one_kwarg_argument=10
    )
    print(env.__dict__)
    print(env.__dir__())
