...
class SargoliniDataTrajectory(Experiment):
    """Data class for sargolini et al.

    2006. https://www.science.org/doi/10.1126/science.1125572 The data
    can be obtained from
    https://archive.norstore.no/pages/public/datasetDetail.jsf?id=8F6BE356-3277-475C-87B1-C7A977632DA7
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            experiment_name=experiment_name, data_url=data_url, paper_url=paper_url
        )
    ...
    def _get_sargolini_data(self, tolerance: float = 1e-10):
        """Load and concatenate animal trajectories from data pre-processed by
        the authors.
        ...
        position: ndarray (n, 2)
            first column is x pos in cm, second column is y pos in cm, n is the number
        ...
        position = (
            np.stack([x_position, y_position], axis=1) * 100
        )  # Convert to cm, originally in meters
        head_direction = np.diff(
            position, axis=0
        )  # Head direction from derivative of position
        head_direction = (
            head_direction
...
class Sargolini2006Data(Hafting2008Data):
    ...
    def __init__(
        ...
        """Sargolini2006Data init, just initializing parent class
        Hafting2008Data.
    ...
    def _find_data_path(self, data_path: str):
        ...
        if data_path is None:
            self.data_path = (
                fetch_data_path("sargolini_2006", subset=self.fetch_subset)
                + "raw_data_sample/"
            )
        else:
    ...
    def _load_data(self):
        ...
        # Initialize data dictionary, later handled by this object itself (so don't
        # worry about this)
            ...
            for sess in sessions:
                ...
                for cell_id in cell_ids:
                    ...
                    if cell_id != "_POS" and cell_id not in ["_EEG", "_EGF"]:
                        try:
                            self.data_per_animal[m_id][sess][session_info] = (
                                cleaned_data["cellTS"]
                            )
                        except Exception:
                            pass
