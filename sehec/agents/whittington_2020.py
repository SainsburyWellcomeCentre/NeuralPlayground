...
class TEM(NeuralResponseModel):
    """
    Implementation for TEM by James C.R.Whittington, Timothy H.Muller, Shirley Mark, Guifen Chen, Caswell Barry, Neil Burgess, Timothy E.J.Behrens
    ...
    Check examples/TEMexample.ipynb
    ----
    Important Attributes
    ---------
    mod_kwargs : dict,
       important parameters described in __init__() funtion and in paramters.py file
    placeholders for tensorflow model:
        self.x: tf.placeholder,
            sensory input from environment
        self.d: tf.placeholder,
            directional information from environment
        self.s_vis: tf.placeholder,
            whether states have been previously visited by the agent
        self.rnn: tf,placeholder,
            hippocampal representation, handled by the Hebbian attractor
        self.g: ndarray,
            entrohinal representation

    Methods
    ---------
    reset(self):
       Reset experiment (clear tensorflow graph and reinitialise attributes and placeholders)
    initialise(self, i):
        Initalise Hebbian weights, variables and sensory objects, returns number of steps in current chunk
    obs_to_state(self, pos, batch):
    ...
    update(self, obs, d, seq_index, it_index):
        Compute the internal (entorhinal) representation while interacting with the environement, save representations
    loop(self, xs, x_s, direcs, gs, index, s_visited):
        Updates all internal representations of TEM and carries out all associated operations
    save_and_plot(self, index, a_rnn, a_rnn_inv):
        Save all internal representations of TEM
    onehot2twohot(self, xs, self.table, self.pars['s_size_comp']):
        converts one-hot encoded sensory input to two-hot encoded vector
    gen_g(g_t, direcs):
        use path integration to generate new entorhinal representation
    inference(g2g_all, xs, x_two_hot, x_t):
        use attractor network to infer both entorhinal and hippocampal representation
    generation(p, g, g_gen):
        generate sensory predictions from memory, inferred g and path-integrated g
    hebbian(p, p_g, p_x):
        update memory representation using Hebbian learning rule
    compute_losses(xs, x_logits_all, g, p, g_gen, p_g, p_x, s_visited):
        calculate losses across all representations of g, p and x
    compute_accuracies(xs, x_all):
    ...
    def __init__(self, model_name="TEM", **mod_kwargs):
        ...
        mod_kwargs : dict
            batch_size: scalar,
                number of environments used simultaneously within a batch
            world_type: string (only 'square' available currently),
                shape of the environments
            poss_behaviours: string (only 'normal' available currently),
                determines whether to include shiny objects of OVC representations
            t_episode: scalar,
                maximum number of timesteps in one chunk
            n_episode: scalar,
                maximum number of chunks in a traning iteration
            train_iters: scalar,
                maximum number of training iterations
            learning_rate_max/min: scalars,
                maximum and minimum learning rate
            stay_still: boolean,
                whether agent can stay still with non-zero probability
            train_on_visited_states_only: boolean,
                whether to only train on visited states
            lambd / eta: floats,
                learning and forgetting rate for Hebbian updates, respectively
            infer_g_type: string (only 'g_p' available currently),
                determines process (inference/generation) used in the p->g process
        """
        ...
        self.x = tf.placeholder(tf.float32, shape=(self.pars['batch_size'], self.pars['s_size'],
                                                   self.pars['t_episode']), name='x')
        ...
        self.acc_p, self.acc_g, self.acc_gt, self.rn = 0, 0, 0, 0
        ...
        self.poss_objects, self.objects, self.n_walk = None, None, None
        self.index = 0
    ...
    def generate_sensorium(self):
        objects = []
        for batch in range(self.pars['batch_size']):
            env_objects = np.zeros(shape=(self.pars['n_states'][batch], self.pars['s_size']))
            ...
            for i in range(self.pars['n_states'][batch]):
                ...
                env_objects[i, :] = self.poss_objects[rand]
            objects.append(env_objects)

        return objects

    def initialise(self, i):
        # INITIALISE ENVIRONMENT AND INPUT VARIABLES
        msg = 'New Environment ' + str(i) + ' ' + str(self.index) + ' ' + str(self.index * self.pars['t_episode'])
        self.logger.info(msg)

        rn = np.random.randint(low=-self.pars['seq_jitter'], high=self.pars['seq_jitter'])
        ...
        if self.save_ticker:
            ...
            self.gs_all, self.ps_all, self.ps_gen_all, self.xs_all, self.gs_timeseries, self.ps_timeseries, self.pos_timeseries = \
                ...
                [None] * self.pars['n_envs_save']
            self.accs_x_to, self.accs_x_from = [None] * self.pars['n_envs_save'], [None] * self.pars['n_envs_save']
            self.n_walk = self.pars['n_save_data']
            ...
            self.check_link_inference, self.positions_link, self.correct_link, self.state_guess = True, [None] * \
                                                                                                  self.pars[
                                                                                                      ...
                                                                                                      None] * self.pars[
                                                                                                      'batch_size']
            self.n_walk = self.pars['link_inf_walk']
        else:
            self.n_walk = self.n_restart + rn
        ...
        self.objects = self.generate_sensorium()

        return int(self.n_walk)
    ...
    def obs_to_states(self, pos, batch):
        ...
        batch: int
            index indicating which environment currently in
    ...
    def act(self, adjs, trans, index, allowed_states):
        """
        ...
        ----------
        observation: ndarray,
            sensory observations for current chunk
        direcs: ndarray,
            actions taken (N, S, E, W) for current chunk
        rewards: ndarray,
        ...
        for batch in range(self.pars['batch_size']):
            ...
            if np.isnan(self.last_states[batch]).any():
                self.state = int(np.random.choice(allowed_states[batch]))
            else:
        ...
        return observations, direcs, rewards

    def update(self, adjs, trans, allowed_states, seq_index, it_index):
        """
        Compute the internal representations (grid and place cells)
        ...
        -----------
        self.n_walk: int,
            number of steps in current chunk
        self.history: ndarray,
            history of agent positions and actions
        """
        if seq_index == 0:
            self.n_walk = self.initialise(it_index)

        obs, d, rewards = self.act(adjs, trans, seq_index, allowed_states)
        ...
        position = np.zeros(shape=(self.pars['batch_size'], self.pars['t_episode'] + 1))

        # Make observations of sensorium in SR states
        for batch in range(self.pars['batch_size']):
            position[batch, 0] = self.obs_to_states(obs[batch, :, 0], batch)
            for step in range(1, self.pars['t_episode'] + 1):
                # state = self.obs_to_states(obs[batch, :, step], batch)
                state = int(obs[batch, :, step])
                observation = self.objects[batch][state]
                position[batch, step] = state
                x[batch, :, step - 1] = observation
        ...
        feed_dict = {self.x: x, self.x_s: self.x_, self.d: d, self.g_: self.gs, self.rnn: self.a_rnn,
                     self.rnn_inv: self.a_rnn_inv, self.it_num: self.index, self.seq_index: seq_index,
                     self.s_vis: s_visited}

        fetch = self.fetches_all_ if self.check_link_inference or self.save_needed else self.fetches_all
        results = self.sess.run(fetch, feed_dict)
        ...
        if self.index % self.pars['save_interval'] == 0 and self.index > 0:
            self.save_ticker = True
        if self.index % self.pars['save_model'] == 0:
            self.save_model = True
        if self.index % self.pars['sum_int'] == 0:
            self.summary_ticker = True

        # feeding in correct initial states
        self.gs, self.x_ = cp.deepcopy(gs[-1]), cp.deepcopy(np.concatenate(x_s[-1], 1))
        self.index += 1

        if seq_index == self.n_walk - 1:
            self.save_and_plot(self.index, a_rnn, a_rnn_inv)

        if seq_index == self.n_walk - 1 and it_index == self.pars['train_iters'] - 1:
            self.train_writer.close()
            self.sess.close()
            print('Finished training')

        return self.n_walk, self.history
    ...
    def loop(self, xs, x_s, direcs, gs, index, s_visited):
        ...
        for i in range(self.pars['t_episode']):
            ...
            # Two-hot Encoding
            x_two_hot = onehot2twohot(xs, tf.convert_to_tensor(self.table))
    ...
    def save_and_plot(self, index, a_rnn, a_rnn_inv):
    ...
    def gen_g(self, g, d):
        """
        Generate next entorhinal representation using path integration alone

        Parameters
        ----------
        g: ndarray,
            previous entorhinal representation
        d: ndarray,
            direction of step taken from previous state

        Returns
        ----------
        g: ndarray,
            new entorhinal representation
        g_inf: ndarray,
            new entorhinal representation, generated for use in inference
        sigma_inf: ndarray,
            variance of entorhinal representation, generated for use in inference
        """
    ...
    def g2g(self, g_old, d, no_direc=False, name=''):
        ...
        g = self.f_g(g)
        # Get variance
        logsig = self.hierarchical_logsig(g_old, 'sig_g2g' + name, self.pars['n_grids_all'], self.pars['n_grids_all'],
    ...
    def inference(self, g2g_all, x, x_two_hot, x_):
        """
        Infer new internal representations, using attractor network

        Parameters
        ----------
        g2g_all: tuple,
            entorhinal representations generated using path integration alone
        x: ndarray,
            sensory observations
        x_two_hot: ndarray,
            compressed sensory observations
        x_: ndarray,
           compressed and filtered sensory observations
        ...
        g: ndarray,
            new entorhinal representation
        p: ndarray,
            new inferred hippocampal representation
        x_s: ndarray,
            compressed sensory observations
        p_x: ndarray,
            memory exracted from attractor network
        """
        # INFER G
        # Infer all variables
    ...
    def generation(self, p, g, g_gen):
        """
        Generate sensory predictions from various internal representations
        ...
        p: ndarray,
            hippocampal memory
        g: ndarray,
            entorhinal representation
        g_gen: ndarray,
            entorhinal representation generated using path integration

        Returns
        ----------
        x_all: ndarray,
            sensory predictions generated from p, inferred g and path integrated g
        x_logits_all: ndarray,
            logits of sensory predictions generated from p, inferred g and path integrated g
        p_g: ndarray,
            hippocampal memory generated from inferred entorhinal representation
        """
    ...
    def hebbian(self, p, p_g, p_x):
        ...
        ----------
        p: ndarray,
            hippocampal memory inferred from observed sensory observation
        p_g: ndarray,
            hippocampal memory inferred from entorhinal representation
        p_x: ndarray,
            hippocampal memory inferred from predicted sensory observation
        """
        a, b = p - p_g, p + p_g
        e, f = None, None
    ...
    def compute_losses(self, x, x_logits_all, g, p, g_gen, p_g, p_x, visited):
        ...
        g_gen: ndarray,
            entorhinal representation generated using path integration
        p_g: ndarray,
            hippocampal memory from inferred entorhinal representation
        p_x: ndarray,
            hippocampal memory from sensory observation
        visited: ndarray,
            whether states have previously been visited by the agent
        """
    ...
    def compute_accuracies(self, x, x_all):
        ...
        x: ndarray,
            sensory observation
        x_all: tuple,
            predicted sensory observations from entorhinal and hippocampal representations
        """
        # Work out accuracies
        x_p, x_g, x_gt = x_all
...
def onehot2twohot(onehot, table):
    """
    ...
    ----------
    onehot: ndarray,
        one-hot encoded vector of sensory observations
    table: ndarray,
        look-up table for converting to two-hot encoded vector

    Returns
    ----------
    twohot: ndarray,
        compressed sensory observation
    """
...
def prepare_data_maps(data, prev_cell_maps, prev_acc_maps, positions, pars):
    ...
    pos_to = position[:, 1:pars['t_episode'] + 1]
    pos_from = position[:, :pars['t_episode']]
