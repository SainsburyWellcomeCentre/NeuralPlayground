...
if load_existing_model:
    ...
    date = '2020-12-20' # 2020-07-05 run 0 for successful node agent
    run = '0'
    i_start = 19999
...
# Extract position data from walks
positions = []
for i, env_data in enumerate(walks):
    ids = []
    for step_data in env_data:
        ids.append(step_data[0]['id'])
    positions.append(ids)
# Create position counts for agent coverage plots
states = [params['n_states_world'][params['diff_env_batches_envs'][env]] for env in range(params['batch_size'])]
pos_count = []
for env in range(len(envs)):
    pos_count.append(np.bincount(positions[env], minlength=states[env]) + 0.001)
...
for i in range(i_start, params['train_it']):
    ...
    if i % params['save_interval'] == 0 and i > 0:
        gs_all, ps_all = [None] * params['n_envs_save'], [None] * params['n_envs_save']
    ...
    if i % params['save_interval'] == 0 and i > 0:
        ...
        np.save(save_path + '/' + 'g_all_' + str(i), gs_all)
        np.save(save_path + '/' + 'p_all_' + str(i), ps_all)
        final_i = i
