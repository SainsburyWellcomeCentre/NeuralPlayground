...
model_spec = importlib.util.spec_from_file_location("model", '../Summaries/' + date + '/torch_run' + run + '/script/model.py')
model = importlib.util.module_from_spec(model_spec)
model_spec.loader.exec_module(model)

# Load the parameters of the model
params = torch.load('../Summaries/' + date + '/torch_run' + run + '/model/params_' + index + '.pt')
# Create a new tem model with the loaded parameters
tem = model.Model(params)
# Load the model weights after training
model_weights = torch.load('../Summaries/' + date + '/torch_run' + run + '/model/tem_' + index + '.pt')
...
envs = list(glob.iglob('../Summaries/' + date + '/torch_run' + run + '/script/envs/*'))
...
# walks = [env.generate_test_walks(n_states=env.n_locations)[0] for env in environments]
...
# model_input = [[[[walks[i][j][k]][0] for i in range(len(walks))] for k in range(3)] for j in range(environments[0].n_locations)]
for i_step, step in enumerate(model_input):
    model_input[i_step][1] = torch.stack(step[1], dim=0)

# torch.save(model_input, 'final_model_input')
...
plt.savefig('accuracy_to_and_from.png')
