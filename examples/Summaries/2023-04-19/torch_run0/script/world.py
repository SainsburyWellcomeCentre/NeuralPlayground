...
class World:
    ...
    def generate_test_walks(self, n_states):
        # Generate walk that steps through all states in order
        walks = [] # This is going to contain a list of (state, observation, action) tuples
        for currWalk in range(n_states):
            new_walk = []
            new_walk = self.walk_test(new_walk, n_states)
            # Clean up walk a bit by only keep essential location dictionary entries
            for step in new_walk[:-1]:
                step[0] = {'id': step[0]['id'], 'shiny': step[0]['shiny']}
            # Append new walk to list of walks
    ...
    def walk_test(self, walk, n_states):
        ...
        actions = [2, 2, 2, 2, 2, 2, 2, 2, 2, 3,
                   ...
                   2, 2, 2, 2, 2, 2, 2, 2, 2, 3,
                   4, 4, 4, 4, 4, 4, 4, 4, 4, 3]
        for curr_step in range(n_states):
            # Get new location based on previous action and location
            new_location = self.locations[states[curr_step]]
